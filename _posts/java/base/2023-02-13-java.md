...
![imgs](/assets/images/java/base/0/image-20210917172215459.png)
...
<img src="/assets/images/java/base/0/image-20220831212422530.png" alt="image-20220831212422530" style="zoom: 67%;" />

编译生成2个class文件：K.class和 Test7.class

<img src="/assets/images/java/base/0/image-20220831212600019.png" alt="image-20220831212600019" style="zoom:67%;" />
...
![image-20220831213322992](/assets/images/java/base/0/image-20220831213322992.png)
...
![img](/assets/images/java/base/0/1362965-20190114161305916-1522316322.png)
...
　　![img](/assets/images/java/base/0/1362965-20190114160755933-897193066.png)
...
![img](/assets/images/java/base/0/1362965-20190114161959489-1682755970.png)
...
![imgs](/assets/images/java/base/0/v2-3d312429710bd6a11eca171858f67751_r.jpg)
...
![imgs](/assets/images/java/base/0/v2-77acfee471d8827a2121157c68f2485d_r.jpg)
...
![imgs](/assets/images/java/base/0/v2-8655d85c8a9df1bc2262485248b07d4d_r.jpg)
...
![imgs](/assets/images/java/base/0/v2-5004d1364ef68f4d71a730f719570dcd_r.jpg)
...
![imgs](/assets/images/java/base/0/v2-a992d2e50fc353edb9027683bf62ee2b_r.jpg)

> StoreLoad 屏障是一个全能型的屏障，它同时具有其他三个屏障的效果。所以执行该屏障开销会很大，因为它使处理器要把缓存中的数据全部刷新到内存中。

下面我们来看看 volatile 读 / 写时是如何插入内存屏障的，见下图：

![imgs](/assets/images/java/base/0/v2-fdead3feef1123c1c225826559def686_r.jpg)
...
![imgs](/assets/images/java/base/0/image-20211107090011799.png)
...
![imgs](/assets/images/java/base/0/image-20211113225644812.png)



#### 子类对象的初始化

![imgs](/assets/images/java/base/0/image-20211113225754840.png)



#### super关键字

![imgs](/assets/images/java/base/0/image-20211113225916289.png)
...
![imgs](/assets/images/java/base/0/image-20211114213148681.png)

![imgs](/assets/images/java/base/0/image-20211114214632680.png)

![imgs](/assets/images/java/base/0/image-20211114214645920.png)



### 静态代码块

![img](/assets/images/java/base/0/20190417170954751.png)
...
![img](/assets/images/java/base/0/20190417174029166.png)
...
![img](/assets/images/java/base/0/20190417174538450.png)

子类：

![img](/assets/images/java/base/0/20190417174554482.png)

在子类中执行main方法，可以看到控制台打印出来的结果

![img](/assets/images/java/base/0/20190417174633290.png)

如果你想看静态代码块的特征，**随着类的加载而执行，而且只执行一次**，就在父类中new一个子类就可以看出来了。

![img](/assets/images/java/base/0/20190417174913631.png)

执行父类中的main方法，控制台打印

![img](/assets/images/java/base/0/20190417175124130.png)
...
![img](/assets/images/java/base/0/20190417180450630.png)
...
![imgs](/assets/images/java/base/0/640.jpg)
...
![imgs](/assets/images/java/base/0/640)
...
空间分配如下图所示：![imgs](/assets/images/java/base/0/640)

**有了用户空间和内核空间，整个linux内部结构可以分为三部分，从最底层到最上层依次是：硬件-->内核空间-->用户空间。**

如下图所示：![imgs](/assets/images/java/base/0/640)
...
![imgs](/assets/images/java/base/0/20190216103001481.png)
...
![imgs](/assets/images/java/base/0/20190216103913801.png)

 第二步 线程A继续执行完成第99次a++，并把值放回内存，此时CPU1中寄存器的值为99，内存中为99；

![imgs](/assets/images/java/base/0/2019021610422627.png)

 第三步 线程B继续执行第一次a++，将其值放回内存，此时CPU2中的寄存器值为1，内存中为1；

![imgs](/assets/images/java/base/0/20190216104439124.png)

 第四步 线程A执行第100次a++，将内存中的值取回CPU1的寄存器，并执行加1，此时CPU1的寄存器中的值为2，内存中为1；

![imgs](/assets/images/java/base/0/20190216104609244.png)

 第五步 线程B执行完剩下的99次操作，并将其放回内存，此时CPU2的寄存器值为100，内存中为100；

![imgs](/assets/images/java/base/0/20190216104806398.png)

 第六步 线程A执行100次操作的最后一部分，将CPU1中的寄存器值2放回内存，将原本的100覆盖,内存中值为2；

![imgs](/assets/images/java/base/0/20190216104954504.png)
