---
layout: post
title: MySQL 获得当前日期时间 函数
subtitle: MySQL 获得当前日期时间 函数
categories: mysql
tags: [mysql]
banner:
...
MySQL `Extract()` 函数除了没有`date()`,`time()` 的功能外，其他功能一应具全。并且还具有选取‘day_microsecond’ 等功能。注意这里不是只选取 day 和 microsecond，而是从日期的 day 部分一直选取到 microsecond 部分。够强悍的吧！
...
## 五、MySQL [时间戳](https://so.csdn.net/so/search?q=时间戳&spm=1001.2101.3001.7020)（Timestamp）函数
